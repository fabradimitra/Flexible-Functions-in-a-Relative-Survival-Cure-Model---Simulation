sim_nn <- function(j,n,tau,cp,lam0,beta0,
                   xies_start,gammas_start,
                   size,tol=1e-5, maxit=1000){
  tryCatch(
    { 
      source("functions/dgps_nn.R", local = TRUE)
      source("functions/em_nn.R", local = TRUE)
      set.seed(j)
      # Generate data
      ones <- rep(1,n)
      age <- rpois(n,45)+20
      ...
      #
      sim <- dgps_nn(n, X, tau, probs, lamd, betad, lam0, beta0, cp)
      cat("\n", j, "Data generated", sep = " ")
      res<-em_nn(t_obs = sim$t_tot,
                 deltas = sim$deltas,
                 X=X,
                 lam0 = lam0,
                 beta0 = beta0,
      ...
      cat(j,
          ...
          res$xies,
          res$xies-xies_true,
          (tail(res$lliks, n=1)[1]-tail(res$lliks, n=2)[1]),
