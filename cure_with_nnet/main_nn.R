...
for (n in c(10000,1000,500,100)){ #
  if (n == 10000){
    cons_check <- as.data.frame(matrix(NA,nrow = 4, ncol = 19))
  ...
  res <- parLapply(cl,1:nsim,sim_nn,
                   n,tau,cp,lam0,beta0,
                   size=4,
  ...
  stopCluster(cl)
  gc()
  assign(paste("Res",n,sep = ""), matrix(unlist(res), nrow = nsim, ncol = 30,byrow=TRUE),env =.GlobalEnv)
  ...
  colnames(fab)<-c("prob0_MAE","prob0_MSE",
                   ...
                   "diff_last","ll_optim","iter", "dif_neg")
  ...
  cons_check[i,] <- c(apply(fab,2,mean)[1],  # mean_MAE_prob0
                      ...
                      apply(fab^2,2,mean)[26],
                      apply(fab,2,sum)[30]) # negative log-lik differences
  i<-i+1
  cat("End iters for n: ", n,"\n", file = "log/log.txt", append = TRUE)
}
#
save.image(file="data/data_nn4.RData") 
