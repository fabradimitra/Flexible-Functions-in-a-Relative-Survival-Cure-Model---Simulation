sim_glm <- function(j,n,tau,cp,lam0,beta0,
                    xies_start,gammas_start,
                    tol=1e-5, maxit=1000){
  tryCatch( 
    { 
      source("functions/dgps_glm.R", local=TRUE)
      source("functions/em_glm.R", local=TRUE)
      set.seed(j)
      # Generate data
      ones <- rep(1,n)
      age <- rpois(n,45)+20
      ...
      #
      #set.seed(NULL)
      sim <-dgps_glm(n, X, tau, probs, lamd, betad, lam0, beta0, cp)
      res<-em_glm(t_obs = sim$t_tot,
                      deltas = sim$deltas,
                      X=X,
                      lam0 = lam0,
                      beta0 = beta0,
